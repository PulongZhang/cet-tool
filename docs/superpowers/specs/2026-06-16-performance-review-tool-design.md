...
SQLite 初始化要求：Flask 应用启动时读取配置中的数据库文件路径；如果未检测到数据库文件，则自动创建 SQLite 数据库并执行建表脚本，完成基础表、索引和必要初始化数据写入；如果数据库文件已存在，则只做 schema 版本检查和必要迁移，不允许删除或重建已有业务数据。
...
### 13.3 SQLite 初始化与自动建表

Flask 应用启动时需要执行数据库初始化检查：

1. 从配置读取 SQLite 数据库文件路径，例如 `data/performance_review.sqlite3`。
2. 如果数据库文件不存在：
   - 自动创建父目录。
   - 创建 SQLite 数据库文件。
   - 执行初始化 SQL，创建业务表、索引、约束和 `schema_version` 版本记录。
   - 写入必要的系统初始化数据，如角色、权限、评价标准静态配置或默认管理员账号。
3. 如果数据库文件已存在：
   - 不重复建表，不清空数据。
   - 检查 `schema_version`，确认当前应用支持该数据库版本。
   - 如后续版本需要调整表结构，通过显式迁移脚本增量执行。
4. 初始化过程必须幂等；重复启动应用不会改变已有业务数据。
5. 初始化或迁移失败时，应用应停止启动并输出明确错误日志，避免在半初始化状态下继续提供服务。
